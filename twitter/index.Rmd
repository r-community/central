...
retweets_by_day <- rstats_retweets %>% 
  ts_data(by = "days") %>% 
  filter(time != as_datetime("2020-05-08")) %>% 
  rename(retweets = n)
...
by_day_metrics <- tweets_by_day %>% 
  ...
  tidyr::replace_na(replace = list(likes = 0))
```
...
by_day_metrics %>%
  ...
  group_by(time = floor_date(time, "month"), weekday) %>%
  summarise(
    tweets = sum(tweets),
    likes = sum(likes),
  ...
  e_timeline_opts(axis_type = "category",
        ...
        let label = date.toLocaleDateString('en-US', options=options);
        return label;
      }"
                    )
                  )) %>%
